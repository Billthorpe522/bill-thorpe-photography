...
function gate(error='',status=200) {
 return page(`<header><a class="brand" href="/">Bill Thorpe<span>.</span></a><span>Private gallery</span></header><main><div class="gate"><div class="eyebrow">Players & families</div><h1>LW vs Andrew</h1><p>${IDS.length} selected photographs from the game.<br>Enter your shared gallery password to view the photographs.</p><form method="post" action="${BASE}unlock"><label for="password">Gallery password</label><input id="password" name="password" type="password" autocomplete="current-password" required maxlength="128" autofocus><button class="unlock" type="submit">View photographs →</button><p class="error" role="alert">${error}</p></form><p class="hint">Need the password? Contact Bill or your team organizer.</p></div></main>`,'',status);
}
function gallery() {
 const items=IDS.map((n,i)=>`<button class="photo" type="button" data-index="${i}" aria-label="Open photograph LwAndrew-${n}"><img src="${BASE}photo/${n}" alt="LW vs Andrew football photograph ${n}" width="${n===245?733:1600}" height="${n===245?1100:1067}" loading="${i<3?'eager':'lazy'}"><span>LWANDREW-${n}</span></button>`);
